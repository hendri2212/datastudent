...
<template>
    ...
    <div class="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-neutral-200 dark:border-neutral-800 pb-4">
            <div>
                <h1 class="text-2xl font-black tracking-tight text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
            ...
            <div class="flex items-center gap-2">
                <Button @click="handleOpenCreateModal" class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white shadow-sm font-semibold">
                    <Plus class="h-4 w-4" />
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
            <div class="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Total Siswa</p>
                    <h3 class="text-2xl font-extrabold text-neutral-900 dark:text-neutral-100">{{ totalStudentsCount }}</h3>
                    <p class="text-[11px] text-emerald-600 font-medium mt-0.5 flex items-center gap-1">
                        <CheckCircle2 class="h-3 w-3" /> {{ verifiedStudentsCount }} Terverifikasi
                    </p>
                </div>
                <div class="p-3 bg-blue-50 dark:bg-blue-950/50 rounded-xl text-blue-600"><Users class="h-6 w-6" /></div>
            </div>

            <div class="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Belum Verifikasi</p>
                    <h3 class="text-2xl font-extrabold text-amber-600">{{ unverifiedStudentsCount }}</h3>
                    <p class="text-[11px] text-neutral-400 mt-0.5">Membutuhkan Tinjauan</p>
                </div>
                <div class="p-3 bg-amber-50 dark:bg-amber-950/50 rounded-xl text-amber-600"><Lock class="h-6 w-6" /></div>
            </div>

            <div class="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Total Prestasi</p>
                    <h3 class="text-2xl font-extrabold text-indigo-600">{{ totalAchievementCount }}</h3>
                    <p class="text-[11px] text-neutral-400 mt-0.5">Penghargaan Siswa</p>
                </div>
                <div class="p-3 bg-indigo-50 dark:bg-indigo-950/50 rounded-xl text-indigo-600"><Trophy class="h-6 w-6" /></div>
            </div>

            <div class="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex items-center justify-between">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-neutral-500">Poin Pelanggaran</p>
                    <h3 class="text-2xl font-extrabold text-rose-600">{{ totalViolationPoints }}</h3>
                    <p class="text-[11px] text-neutral-400 mt-0.5">Akumulasi Poin</p>
                </div>
                <div class="p-3 bg-rose-50 dark:bg-rose-950/50 rounded-xl text-rose-600"><Shield class="h-6 w-6" /></div>
            </div>

            <div class="bg-white dark:bg-neutral-900 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm flex flex-col justify-between col-span-2 lg:col-span-1">
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-wider text-neutral-500 mb-1">Distribusi Gender</p>
                    <div class="flex flex-wrap gap-2 text-xs font-semibold">
                        <span v-for="(count, gender) in genderStats" :key="gender" class="bg-neutral-100 dark:bg-neutral-800 px-2 py-1 rounded-md text-neutral-700 dark:text-neutral-300">
                            {{ gender }}: <strong class="text-blue-600">{{ count }}</strong>
                        </span>
                    </div>
                </div>
                <div v-if="activeFilterCount > 0" class="mt-2">
                    <Badge variant="secondary" class="text-[10px] bg-blue-50 text-blue-700 border border-blue-200">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-200 dark:border-neutral-800 pb-2">
            <div class="flex items-center gap-4 text-sm font-medium">
                <button
                    type="button"
                    @click="activeTab = 'active'"
                    :class="['pb-2 border-b-2 transition-all flex items-center gap-2', activeTab === 'active' ? 'border-blue-600 text-blue-600 font-bold' : 'border-transparent text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300']"
                >
                ...
                <button
                    ...
                    :class="['pb-2 border-b-2 transition-all flex items-center gap-2', activeTab === 'trashed' ? 'border-red-600 text-red-600 font-bold' : 'border-transparent text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300']"
                >
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" size="sm" @click="isFilterModalOpen = true" class="h-9 text-xs border-neutral-300 flex items-center gap-1.5">
                    <Filter class="h-3.5 w-3.5 text-neutral-500" /> 
                    <span>Filter</span>
                    <Badge v-if="activeFilterCount > 0" class="ml-1 h-4 px-1 text-[9px] bg-blue-600 text-white rounded-full">{{ activeFilterCount }}</Badge>
                </Button>
                <Button v-if="activeFilterCount > 0 || searchQuery" variant="ghost" size="sm" @click="handleResetFilters" class="h-9 text-xs text-rose-600 hover:bg-rose-50">
                    <RefreshCw class="h-3.5 w-3.5 mr-1" /> Reset
                </Button>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead class="bg-neutral-50 dark:bg-neutral-800/50 text-neutral-600 dark:text-neutral-400 font-semibold border-b border-neutral-200 dark:border-neutral-800">
                        <tr>
                            <th class="p-3 pl-4 text-xs tracking-wide uppercase">Siswa & Biodata</th>
                            <th class="p-3 text-xs tracking-wide uppercase">Akademik</th>
                            <th class="p-3 text-xs tracking-wide uppercase">Kontak & Alamat</th>
                            <th class="p-3 text-xs tracking-wide uppercase">Status & Verifikasi</th>
                            <th class="p-3 pr-4 text-xs tracking-wide uppercase text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
                        <tr v-if="studentList.length === 0">
                            <td colspan="5" class="p-12 text-center text-neutral-500 text-sm">
                        ...
                        <tr v-for="student in studentList" :key="student.id" class="hover:bg-blue-50/40 dark:hover:bg-neutral-800/40 transition-colors">
                            <td class="p-3 pl-4">
                                <div class="flex items-center gap-3">
                                    <div class="h-9 w-9 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 font-bold flex items-center justify-center text-xs border border-blue-200">
                                        {{ student.full_name?.charAt(0)?.toUpperCase() || 'S' }}
                                    </div>
                                    <div>
                                        <div class="font-bold text-neutral-900 dark:text-neutral-100 text-[13px] flex items-center gap-1.5">
                                            <span>{{ student.full_name }}</span>
                                            <span v-if="student.nickname" class="text-neutral-400 font-normal italic text-xs">({{ student.nickname }})</span>
                                        </div>
                                        <div class="text-[11px] text-neutral-500 flex items-center gap-1.5 mt-0.5">
                            ...
                            <td class="p-3">
                                <div class="flex items-center gap-1.5 mb-1">
                                    <Badge variant="outline" class="text-[10px] font-bold bg-blue-50 text-blue-700 border-blue-200 px-2 py-0.5 rounded-md">
                            ...
                            <td class="p-3">
                                <div class="text-[12px] font-medium text-neutral-800 dark:text-neutral-200 flex items-center gap-1">
                                    <Phone class="h-3 w-3 text-neutral-400" />
                                    <span>{{ student.phone || '-' }}</span>
                                </div>
                                <div class="text-[11px] text-neutral-500 truncate max-w-[160px] flex items-center gap-1 mt-0.5" :title="student.email || '-'">
                            ...
                            <td class="p-3">
                                <div class="flex items-center gap-1.5 mb-1">
                                    <div :class="['w-2 h-2 rounded-full', isStudentVerified(student) ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500']"></div>
                                    <span :class="['text-[11px] font-semibold', isStudentVerified(student) ? 'text-emerald-700 dark:text-emerald-400' : 'text-amber-700 dark:text-amber-400']">
                                        {{ isStudentVerified(student) ? 'Terverifikasi' : 'Belum Diverifikasi' }}
                                    </span>
                                </div>
                                <div class="text-[11px] text-neutral-500">{{ student.student_status?.name || 'Status (-)' }}</div>
                            </td>
                            <td class="p-3 pr-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <Button variant="ghost" size="icon" @click="handleOpenDetailModal(student)" title="Lihat Detail Lengkap" class="h-8 w-8 text-blue-600 hover:bg-blue-50 rounded-lg">
                                        <Eye class="h-4 w-4" />
                                    </Button>

                                    <template v-if="activeTab === 'active'">
                                        <Button v-if="!isStudentVerified(student)" variant="ghost" size="icon" @click="handleVerifyStudent(student)" title="Verifikasi Data Siswa" class="h-8 w-8 text-emerald-600 hover:bg-emerald-50 rounded-lg">
                                            <Check class="h-4 w-4" />
                                        </Button>
                                        <Button v-else variant="ghost" size="icon" @click="handleUnverifyStudent(student)" title="Batalkan Verifikasi" class="h-8 w-8 text-slate-500 hover:bg-slate-100 rounded-lg">
                                            <Unlock class="h-4 w-4" />
                                        </Button>

                                        <Button variant="ghost" size="icon" @click="handleOpenEditModal(student)" title="Edit Data" class="h-8 w-8 text-amber-600 hover:bg-amber-50 rounded-lg">
                                            <Pencil class="h-4 w-4" />
                                        </Button>
                                        <Button variant="ghost" size="icon" @click="handleOpenDeleteModal(student)" title="Hapus Siswa" class="h-8 w-8 text-rose-600 hover:bg-rose-50 rounded-lg">
                                            <Trash2 class="h-4 w-4" />
                                        </Button>
                                    </template>
                                    <template v-else>
                                        <Button variant="ghost" size="icon" @click="handleRestore(student)" title="Pulihkan Data" class="h-8 w-8 text-emerald-600 hover:bg-emerald-50 rounded-lg">
                                            <UserCheck class="h-4 w-4" />
                                        </Button>
                                        <Button variant="ghost" size="icon" @click="handleForceDelete(student)" title="Hapus Permanen" class="h-8 w-8 text-rose-600 hover:bg-rose-50 rounded-lg">
                                            <Trash2 class="h-4 w-4" />
                                        </Button>
                                    </template>
                                </div>
            ...
            <div v-if="paginationMeta && paginationMeta.links" class="p-4 border-t border-neutral-200 dark:border-neutral-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-neutral-500">
                <div>
    ...
    <Dialog :open="isFilterModalOpen" @update:open="isFilterModalOpen = $event">
        <DialogContent class="sm:max-w-lg">
            ...
            <DialogFooter class="flex justify-between items-center gap-2 pt-3 border-t dark:border-neutral-800">
                <Button variant="ghost" size="sm" @click="handleResetFilters">Reset Filter</Button>
                <Button size="sm" @click="isFilterModalOpen = false" class="bg-blue-600 text-white">Terapkan Filter</Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
    ...
    <Dialog :open="isDetailModalOpen" @update:open="isDetailModalOpen = $event">
        <DialogContent class="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader class="border-b pb-3 dark:border-neutral-800">
                <DialogTitle class="text-xl flex items-center justify-between">
                    <span class="flex items-center gap-2">
                        <User class="h-5 w-5 text-blue-600" /> Profil & Data Lengkap Siswa
                    </span>
                </DialogTitle>
                <DialogDescription>
            ...
            <div v-if="selectedStudent" class="space-y-6 pt-2">
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-neutral-800 dark:to-neutral-900 p-4 rounded-xl border border-blue-100 dark:border-neutral-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 mt-2">
                            <Badge class="bg-blue-600 text-white">{{ selectedStudent.classroom?.name || 'Tanpa Kelas' }}</Badge>
                            <Badge variant="secondary">{{ selectedStudent.major?.name || 'Umum' }}</Badge>
                            <Badge variant="outline">{{ selectedStudent.school?.name || 'Sekolah (-)' }}</Badge>
                        </div>
                    </div>
                    <Button variant="outline" size="sm" @click="copySummaryText(selectedStudent)" class="bg-white dark:bg-neutral-800 flex items-center gap-1.5 self-start md:self-auto shadow-sm">
                        <Check v-if="copiedKey === 'summary_' + selectedStudent.id" class="h-4 w-4 text-emerald-600" />
                        <Copy v-else class="h-4 w-4" />
                        <span>{{ copiedKey === 'summary_' + selectedStudent.id ? 'Tersalin!' : 'Salin Seluruh Profil' }}</span>
                    </Button>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div class="border p-3.5 rounded-xl flex items-center justify-between bg-neutral-50/50 dark:bg-neutral-900 dark:border-neutral-800">
                        <div>
                            <span class="text-[10px] text-neutral-500 uppercase font-bold tracking-wider">Nomor Induk Siswa Nasional (NISN)</span>
                            <p class="font-mono text-base font-bold text-blue-600 dark:text-blue-400 mt-0.5">{{ selectedStudent.nisn || '-' }}</p>
                        </div>
                        <Button variant="ghost" size="sm" @click="copyToClipboard(selectedStudent.nisn, 'nisn')" :disabled="!selectedStudent.nisn">
                    ...
                    <div class="border p-3.5 rounded-xl flex items-center justify-between bg-neutral-50/50 dark:bg-neutral-900 dark:border-neutral-800">
                        <div>
                            <span class="text-[10px] text-neutral-500 uppercase font-bold tracking-wider">Nomor Induk Siswa (NIS)</span>
                            <p class="font-mono text-base font-bold text-neutral-800 dark:text-neutral-200 mt-0.5">{{ selectedStudent.nis || '-' }}</p>
                        </div>
                        <Button variant="ghost" size="sm" @click="copyToClipboard(selectedStudent.nis, 'nis')" :disabled="!selectedStudent.nis">
                ...
                <div class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <User class="h-4 w-4 text-blue-500" /> Identitas & Biodata Pribadi
                    </h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
                        <div class="p-3 border rounded-lg dark:border-neutral-800">
                ...
                <div class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <MapPin class="h-4 w-4 text-emerald-500" /> Kontak & Alamat Tempat Tinggal
                    </h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        <div class="p-3 border rounded-lg dark:border-neutral-800 flex justify-between items-center">
                            <div>
                        ...
                        <div class="p-3 border rounded-lg dark:border-neutral-800 flex justify-between items-center">
                            <div>
                                ...
                                <span class="font-semibold text-neutral-800 dark:text-neutral-200">{{ selectedStudent.email || '-' }}</span>
                            </div>
                        ...
                        <div class="p-3 border rounded-lg dark:border-neutral-800 sm:col-span-2">
                ...
                <div class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <Building2 class="h-4 w-4 text-indigo-500" /> Lembaga Pendidikan & Akademik
                    </h4>
                ...
                <div v-if="selectedStudent.family" class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <Users class="h-4 w-4 text-purple-500" /> Data Orang Tua & Wali
                    </h4>
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-3 text-xs">
                        <div class="p-3 border rounded-lg dark:border-neutral-800 bg-neutral-50/30 dark:bg-neutral-900/40 space-y-1">
                        ...
                        <div class="p-3 border rounded-lg dark:border-neutral-800 lg:col-span-3 space-y-1">
                            <span class="text-rose-600 font-bold block border-b pb-1">KONTAK DARURAT & CATATAN</span>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                                <p><span class="text-neutral-400">Nama Kontak Darurat:</span> {{ selectedStudent.family.emergency_contact_name || '-' }}</p>
                                <p><span class="text-neutral-400">No. HP Darurat:</span> {{ selectedStudent.family.emergency_contact_phone || '-' }}</p>
                                <p><span class="text-neutral-400">Hubungan Keluarga:</span> {{ selectedStudent.family.relationship_type?.name || '-' }}</p>
                            </div>
                ...
                <div v-if="selectedStudent.health" class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <Heart class="h-4 w-4 text-red-500" /> Rekam Kesehatan Siswa
                    </h4>
                ...
                <div v-if="selectedStudent.education_histories?.length" class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <GraduationCap class="h-4 w-4 text-amber-500" /> Riwayat Pendidikan Sebelumnya
                    </h4>
                    <div class="space-y-2 text-xs">
                        <div v-for="(edu, index) in selectedStudent.education_histories" :key="edu.id || index" class="p-3 border rounded-lg dark:border-neutral-800 bg-neutral-50/30 dark:bg-neutral-900/30">
                            <div class="flex items-center justify-between gap-2">
                                <div>
                                    <p class="font-bold text-sm text-neutral-900 dark:text-neutral-100">{{ edu.school_name || '-' }}</p>
                                    <p class="text-neutral-500 text-[11px]">{{ edu.education_level?.name || 'Jenjang tidak ditentukan' }}</p>
                                </div>
                                <Badge :variant="edu.is_graduated ? 'default' : 'secondary'">{{ edu.is_graduated ? 'Lulus' : 'Belum Lulus' }}</Badge>
                            </div>
                            <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px] text-neutral-600 dark:text-neutral-400">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                        <div v-for="(soc, index) in selectedStudent.socials" :key="soc.id || index" class="p-2.5 border rounded-lg dark:border-neutral-800 flex items-center justify-between gap-2">
                            <div>
                                ...
                                <p class="text-neutral-500 text-[11px] font-mono">{{ soc.username || soc.url || '-' }}</p>
                                <a 
                                    :href="formatSocialUrl(soc.url, soc.username, soc.social_platform?.name)" 
                                    target="_blank" 
                                    rel="noreferrer noopener"
                                    class="text-blue-600 hover:underline text-[11px] font-medium flex items-center gap-1 mt-1 truncate"
                                >
                                    <span>{{ soc.url || 'Buka Profil Media Sosial' }}</span>
                                    <ExternalLink class="h-3 w-3 shrink-0" />
                                </a>
                            </div>
                            <Button v-if="soc.url" variant="ghost" size="icon" class="h-6 w-6" @click="copyToClipboard(soc.url, 'soc_' + index)">
                ...
                <div v-if="selectedStudent.achievements?.length" class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <Trophy class="h-4 w-4 text-amber-500" /> Prestasi Siswa
                    </h4>
                    <div class="space-y-2 text-xs">
                        <div v-for="(ach, index) in selectedStudent.achievements" :key="ach.id || index" class="p-3 border rounded-lg dark:border-neutral-800 bg-amber-50/20 dark:bg-amber-950/10">
                            <div class="flex items-center justify-between">
                                <p class="font-bold text-neutral-900 dark:text-neutral-100">{{ ach.title }}</p>
                                <Badge variant="outline" class="bg-amber-100 text-amber-800 border-amber-300">Peringkat: {{ ach.rank || '-' }}</Badge>
                            </div>
                            <p class="text-neutral-500 text-[11px] mt-1">
                                Penyelenggara: {{ ach.organizer || '-' }} | Tingkat: {{ ach.level || '-' }} | Tanggal: {{ ach.achievement_date || '-' }}
                            </p>
                            <p v-if="ach.description" class="text-neutral-600 dark:text-neutral-300 text-[11px] mt-1">{{ ach.description }}</p>
                        </div>
                    </div>
                </div>

                <div v-if="selectedStudent.violations?.length" class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <Shield class="h-4 w-4 text-rose-500" /> Catatan Pelanggaran Kedisiplinan
                    </h4>
                    <div class="space-y-2 text-xs">
                        <div v-for="(vio, index) in selectedStudent.violations" :key="vio.id || index" class="p-3 border rounded-lg dark:border-neutral-800 bg-rose-50/20 dark:bg-rose-950/10">
                            <div class="flex items-center justify-between">
                                <p class="font-bold text-neutral-900 dark:text-neutral-100">{{ vio.title }}</p>
                                <Badge variant="destructive">Poin: {{ vio.point ?? '-' }}</Badge>
                            </div>
                            <p class="text-neutral-500 text-[11px] mt-1">Tanggal: {{ vio.violation_date || '-' }}</p>
                            <p v-if="vio.description" class="text-neutral-600 dark:text-neutral-300 text-[11px] mt-1">{{ vio.description }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="selectedStudent.documents?.length" class="space-y-3">
                    <h4 class="text-xs font-bold uppercase tracking-wider text-neutral-500 flex items-center gap-1.5 border-b pb-1">
                        <FileText class="h-4 w-4 text-blue-500" /> Berkas & Dokumen Resmi
                    </h4>
                    <div class="space-y-2 text-xs">
                        <div v-for="doc in selectedStudent.documents" :key="doc.id" class="p-3 border rounded-lg dark:border-neutral-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div>
                                <p class="font-bold text-neutral-900 dark:text-neutral-100">{{ doc.original_name || doc.stored_name || 'Dokumen' }}</p>
                                <p class="text-neutral-500 text-[11px]">{{ doc.document_type?.name || 'Tipe Berkas' }} • {{ doc.mime_type || '-' }}</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <a :href="getDocumentPreviewUrl(doc)" target="_blank" class="px-3 py-1 border rounded-md text-xs font-medium hover:bg-neutral-100 dark:hover:bg-neutral-800">Pratinjau</a>
                                <a :href="getDocumentDownloadUrl(doc)" target="_blank" class="px-3 py-1 bg-blue-600 text-white rounded-md text-xs font-medium hover:bg-blue-700">Unduh</a>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="p-3 border rounded-lg dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 text-xs flex flex-wrap justify-between items-center gap-2">
                    <div class="flex items-center gap-2">
            ...
            <DialogFooter class="mt-4 border-t pt-3 dark:border-neutral-800">
                <Button variant="outline" @click="isDetailModalOpen = false">Tutup Modal</Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
    ...
    <Dialog :open="isDeleteModalOpen" @update:open="isDeleteModalOpen = $event">
        <DialogContent class="sm:max-w-md">
            <DialogHeader>
                <DialogTitle class="text-rose-600 font-bold flex items-center gap-2">
            ...
            <DialogFooter class="mt-4 flex gap-2 justify-end">
                ...
                <Button variant="destructive" :disabled="isSubmitting" @click="handleDelete" class="bg-rose-600 hover:bg-rose-700 text-white">

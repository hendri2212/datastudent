...
type ClassroomLevel = 'X' | 'XI' | 'XII';
type ClassroomStatus = 'Aktif' | 'Nonaktif';
type ClassroomTab = 'active' | 'trashed';
...
// Filter States
const activeTab = ref<ClassroomTab>(
    props.filters?.tab === 'trashed' ? 'trashed' : 'active',
);
...
const editForm = useForm({
    ...
    level: 'X' as ClassroomLevel,
    rombel: 1,
    status: 'Aktif' as ClassroomStatus,
});
...
const filteredClassrooms = computed(() => {
    return props.classrooms.filter((item) => {
        ...
        const matchesMajor =
            !selectedMajor.value ||
            item.major?.id.toString() === selectedMajor.value;
        const matchesRombel =
            !selectedRombel.value ||
            item.rombel.toString() === selectedRombel.value;
...
const totalPages = computed(
    () => Math.ceil(filteredClassrooms.value.length / itemsPerPage.value) || 1,
);
...
const endIndex = computed(() =>
    Math.min(
        ...
        filteredClassrooms.value.length,
    ),
);
...
// Switch Tab Handler
const switchTab = (tab: ClassroomTab) => {
    activeTab.value = tab;
    currentPage.value = 1;
    router.get(
        '/classrooms',
        { tab, search: searchQuery.value },
        { preserveState: true },
    );
};
            ...
            >
                <div>
                    ...
                    <p class="text-sm text-neutral-500 dark:text-neutral-400">
                        Kelola rombel kelas dengan penomoran otomatis dan fitur
                        tempat sampah.
                    </p>
                </div>
                ...
                >
                    ...
                    >
                        <span>
                            ...
                            <strong>{{ filteredClassrooms.length }}</strong>
                            data
                        </span>
    ...
    <Dialog v-model:open="isAddModalOpen">
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Pilih tingkat dan jurusan. Nomor rombel akan dibuat secara
                    otomatis.
                </DialogDescription>
            </DialogHeader>
    ...
    <Dialog v-model:open="isDeleteModalOpen">
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    Kelas <strong>{{ selectedClassroom?.name }}</strong> akan
        ...
        <DialogContent>
            <DialogHeader>
                ...
                <DialogDescription>
                    ...
                    permanen? Data tidak dapat dikembalikan.
                </DialogDescription>
            </DialogHeader>
            <DialogFooter>
                <Button
